...
export default defineComponent({
  ...
  setup(props) {
    ...
    return () => <Spin class="user-table" spinning={loading.value}>
      <Form layout='inline' class="user-table__search">
        <Form.Item label="姓名">
          <Input
            v-model={[searchName.value, 'value']}
            allowClear
            placeholder="请输入姓名"
          />
        </Form.Item>

        <Form.Item noStyle>{list.value.length}/{dataSource.value.length}</Form.Item>
      </Form>
